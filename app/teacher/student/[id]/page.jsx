...
export default function StudentDetailsPage() {
    ...
    const currentSurah = quranData.find(s => s.id === (student?.currentHifzSurahId || 114));
    const allowedPages = (() => {
        const basePages = getSurahPages(currentSurah?.id || 114);
        // If beginner at Fatiha, also allow jumping to An-Nas (604)
        if (currentSurah?.id === 1 && student?.juzCount === 0) {
            return [...new Set([...basePages, 604])].sort((a,b)=>a-b);
        }
        return basePages;
    })();
                                                                        ...
                                                                        return (
                                                                            <div key={s.id} className={`p-5 rounded-[2rem] border-2 transition-all duration-300 relative overflow-hidden ${evalData.status === 'PERFECT' ? 'border-emerald-200 bg-gradient-to-b from-emerald-50/50 to-emerald-100/30 dark:border-emerald-800 dark:from-emerald-900/20 dark:to-emerald-900/10 shadow-emerald-100/50' : evalData.status === 'REVIEW' ? 'border-amber-200 bg-gradient-to-b from-amber-50/50 to-amber-100/30 dark:border-amber-800 dark:from-amber-900/20 dark:to-amber-900/10 shadow-amber-100/50' : 'border-red-200 bg-gradient-to-b from-red-50/50 to-red-100/30 dark:border-red-800 dark:from-red-900/20 dark:to-red-900/10 shadow-red-100/50'} shadow-lg`}>
                                                                                ...
                                                                                <div className="mb-5 flex flex-col items-center gap-4">
                                                                                    ...
                                                                                    <div className="flex gap-3 bg-white/80 dark:bg-slate-800 p-1.5 rounded-full shadow-inner border border-slate-200 dark:border-slate-700" dir="ltr">
                                                                                        <button type="button" onClick={() => setHifzSurahEvals(p => ({...p, [s.id]: {...evalData, status: 'FAILED'}}))} className={`w-10 h-10 rounded-full flex items-center justify-center text-lg transition-all duration-300 ${evalData.status === 'FAILED' ? 'bg-red-500 text-white shadow-[0_0_15px_rgba(239,68,68,0.5)] scale-110 opacity-100' : 'bg-white dark:bg-slate-700 shadow-sm opacity-40 hover:opacity-100 hover:scale-105 hover:bg-red-50 dark:hover:bg-slate-600'}`}>❌</button>
                                                                                        <button type="button" onClick={() => setHifzSurahEvals(p => ({...p, [s.id]: {...evalData, status: 'REVIEW'}}))} className={`w-10 h-10 rounded-full flex items-center justify-center text-lg transition-all duration-300 ${evalData.status === 'REVIEW' ? 'bg-amber-500 text-white shadow-[0_0_15px_rgba(245,158,11,0.5)] scale-110 opacity-100' : 'bg-white dark:bg-slate-700 shadow-sm opacity-40 hover:opacity-100 hover:scale-105 hover:bg-amber-50 dark:hover:bg-slate-600'}`}>⚠️</button>
                                                                                        <button type="button" onClick={() => setHifzSurahEvals(p => ({...p, [s.id]: {...evalData, status: 'PERFECT'}}))} className={`w-10 h-10 rounded-full flex items-center justify-center text-lg transition-all duration-300 ${evalData.status === 'PERFECT' ? 'bg-emerald-500 text-white shadow-[0_0_15px_rgba(16,185,129,0.5)] scale-110 opacity-100' : 'bg-white dark:bg-slate-700 shadow-sm opacity-40 hover:opacity-100 hover:scale-105 hover:bg-emerald-50 dark:hover:bg-slate-600'}`}>✔️</button>
                                                                                    </div>
                                                                                </div>
