...
function TeacherWeeklyReportContent() {
    ...
    const [endDate, setEndDate] = useState(() => {
        ...
        wed.setDate(sun.getDate() + 3); // To Wednesday
        ...
        <div className="min-h-screen bg-white font-noto p-8 text-right" dir="rtl">
            ...
            <div className="bg-slate-50 rounded-[2rem] p-8 mb-8 border border-slate-100 shadow-sm print:shadow-none print:border-none print:bg-transparent print:p-0">
                ...
                <div className="flex gap-4 mb-8 flex-wrap">
                    <div className="bg-white px-6 py-4 rounded-2xl shadow-sm border border-slate-100 flex flex-col items-center min-w-[110px] print:border-slate-300 print:bg-white text-slate-800"><span className="text-slate-400 text-xs font-bold mb-1">عدد الطلاب</span><span className="text-2xl font-black">{reportData.length}</span></div>
                    <div className="bg-white px-6 py-4 rounded-2xl shadow-sm border border-slate-100 flex flex-col items-center min-w-[110px] print:border-slate-300 print:bg-white text-emerald-600"><span className="text-slate-400 text-xs font-bold mb-1">إجمالي الحضور</span><span className="text-2xl font-black">{totalPresent}</span></div>
                    <div className="bg-white px-6 py-4 rounded-2xl shadow-sm border border-slate-100 flex flex-col items-center min-w-[110px] print:border-slate-300 print:bg-white text-rose-500"><span className="text-slate-400 text-xs font-bold mb-1">إجمالي الغياب</span><span className="text-2xl font-black">{totalAbsent}</span></div>
                    <div className="bg-white px-6 py-4 rounded-2xl shadow-sm border border-slate-100 flex flex-col items-center min-w-[110px] print:border-slate-300 print:bg-white text-amber-500"><span className="text-slate-400 text-xs font-bold mb-1">إجمالي التأخر</span><span className="text-2xl font-black">{totalLate}</span></div>
                    <div className="bg-white px-6 py-4 rounded-2xl shadow-sm border border-slate-100 flex flex-col items-center min-w-[110px] print:border-slate-300 print:bg-white text-indigo-600"><span className="text-slate-400 text-xs font-bold mb-1">الأوجه المقروءة</span><span className="text-2xl font-black">{totalPages}</span></div>
                </div>
                                ...
                                {sortedData.map((student, idx) => {
                                    ...
                                    return (
                                        <tr key={student.id} className={idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/40'}>
                                            <td className="p-4 font-bold text-slate-800 border-r border-slate-50 print:border-slate-200">{student.name}<div className="text-[10px] text-slate-400 font-normal mt-0.5">{student.hifzProgress || 'لم يُحدَّد بعد'}</div></td>
                                            <td className="p-4 text-center border-r border-slate-50 print:border-slate-200"><span className="inline-flex items-center justify-center min-w-[2rem] h-8 px-2 bg-emerald-50 text-emerald-700 font-black rounded-lg text-sm print:bg-transparent">{att.present}</span></td>
                                            <td className="p-4 text-center border-r border-slate-50 print:border-slate-200"><span className={`inline-flex items-center justify-center min-w-[2rem] h-8 px-2 font-black rounded-lg text-sm print:bg-transparent ${absTotal > 0 ? 'bg-rose-50 text-rose-600' : 'bg-slate-50 text-slate-400'}`}>{absTotal > 0 ? absTotal : '—'}</span></td>
                                            <td className="p-4 text-center border-r border-slate-50 print:border-slate-200"><span className={`inline-flex items-center justify-center min-w-[2rem] h-8 px-2 font-black rounded-lg text-sm print:bg-transparent ${att.late > 0 ? 'bg-amber-50 text-amber-600' : 'bg-slate-50 text-slate-400'}`}>{att.late > 0 ? att.late : '—'}</span></td>
                                            <td className="p-4 text-center border-r border-slate-50 print:border-slate-200"><span className={`inline-flex items-center justify-center min-w-[2rem] h-8 px-2 font-black rounded-lg text-sm print:bg-transparent ${hifzDays > 0 ? 'bg-emerald-50 text-emerald-700' : 'text-slate-300'}`}>{hifzDays > 0 ? `${hifzDays} أيام` : '—'}</span></td>
                                            <td className="p-4 text-center border-r border-slate-50 print:border-slate-200"><span className={`inline-flex items-center justify-center min-w-[2rem] h-8 px-2 font-black rounded-lg text-sm print:bg-transparent ${majorDays > 0 ? 'bg-indigo-50 text-indigo-700' : 'text-slate-300'}`}>{majorDays > 0 ? `${majorDays} أيام` : '—'}</span></td>
                                            <td className="p-4 text-center border-r border-slate-50 print:border-slate-200"><span className={`inline-flex items-center justify-center min-w-[2rem] h-8 px-2 font-black rounded-lg text-sm print:bg-transparent ${minorDays > 0 ? 'bg-blue-50 text-blue-700' : 'text-slate-300'}`}>{minorDays > 0 ? `${minorDays} أيام` : '—'}</span></td>
                                            <td className="p-4 text-center"><span className="inline-flex items-center justify-center h-8 px-3 bg-slate-100 text-slate-700 font-black rounded-lg text-sm print:bg-transparent print:border print:border-slate-300">{totalStudentPages}</span></td>
                                        </tr>
                                    );
                                })}
                            </tbody>
            ...
            <div className="mt-4 text-xs text-slate-400 flex justify-between px-2 print:text-black"><span>تم استخراج هذا التقرير بتاريخ {printDate}</span><span>منصة تحفيظ القرآن الكريم</span></div>
            <style jsx global>{` @media print { @page { size: A4 landscape; margin: 0.8cm; } .no-print { display: none !important; } body { background: white !important; -webkit-print-color-adjust: exact; print-color-adjust: exact; } } `}</style>
        </div>
    );
}
